...
function Navbar() {
  ...
  return (
    <div className="top-0 w-full h-16 shadow-2xl fixed px-4 md:px-8 flex justify-between bg-white z-50">
      <div className="flex h-16 items-center cursor-pointer text-2xl font-bold" onClick={()=>navigate("/home")}>
        Storage System
      </div>
      {
        user.name?
        <div className="items-center hidden sm:flex">
          <div className="dropdown">
            ...
            <div className="dropdown-content">
              <div onClick={()=>{navigate('/peralatan')}}>List Peralatan</div>
              <div>Kategori - Admin -</div>
            </div>
          </div>
          <div className="dropdown">
          ...
          <div className="dropdown">
            <button className="dropbtn w-[200px] text-2xl">Hi, {user.name}</button>
            <div className="dropdown-content">
              <div onClick={logout}>Logout</div>
              <div>Manage User -Admin-</div>
            </div>
          </div>
        </div>
        :
